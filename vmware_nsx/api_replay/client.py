...
class ApiReplayClient(object):
    ...
    def migrate_qos_rule(self, dest_policy, source_rule):
        ...
        try:
            if rule_type == 'bandwidth_limit':
                ...
                LOG.info(_LI("QoS rule type %(rule)s is not supported for "
                             "policy %(pol)s"),
    ...
    def migrate_security_groups(self):
        ...
        LOG.info(_LI("Migrating %s security groups"), total_num)
            ...
            # dest server doesn't have the group so we create it here.
                ...
                try:
                    ...
                    new_sg = self.dest_neutron.create_security_group(
                        {'security_group': body})
                    LOG.info(_LI("Created security-group %(count)s/%(total)s: "
                                 "%(sg)s"),
    ...
    def migrate_routers(self):
        ...
        LOG.info(_LI("Migrating %s routers"), total_num)
            ...
            if dest_router is False:
                ...
                try:
                    new_router = (self.dest_neutron.create_router(
                        {'router': body}))
                    LOG.info(_LI("created router %(count)s/%(total)s: "
                                 "%(rtr)s"),
                             {'count': count, 'total': total_num,
                              'rtr': new_router})
                except Exception as e:
                    LOG.error(_LE("Failed to create router %(rtr)s: %(e)s"),
                              {'rtr': router, 'e': e})
    ...
    def migrate_routers_routes(self, routers_routes):
        ...
        LOG.info(_LI("Migrating %s routers routes"), total_num)
        for count, (router_id, routes) in enumerate(
            six.iteritems(routers_routes), 1):
            try:
                self.dest_neutron.update_router(router_id,
                    {'router': {'routes': routes}})
                LOG.info(_LI("Added routes to router %(rtr)s %(count)s/"
                             "%(total)s:"),
                         {'count': count, 'total': total_num,
                          'rtr': router_id})
            except Exception as e:
                LOG.error(_LE("Failed to add routes %(routes)s to router "
                              "%(rtr)s: %(e)s"),
        ...
        for pool in source_subnetpools:
            ...
            if pool['is_default']:
                ...
                try:
                    ...
                    LOG.error(_LE("Failed to create subnetpool %(pool)s: "
                                  "%(e)s"),
                              {'pool': pool, 'e': e})
        return subnetpools_map
    ...
    def fix_port(self, body):
        ...
        if (body.get('mac_learning_enabled') and
            ...
            LOG.warning(_LW("Disabling port security of port %s: The plugin "
        ...
        if body.get('router:external'):
            ...
            if fields_reset:
                LOG.warning(_LW("Ignoring provider network fields while "
                                "migrating external network %s"), body['id'])
    ...
    def migrate_networks_subnets_ports(self, routers_gw_info):
        ...
        LOG.info(_LI("Migrating %(nets)s networks, %(subnets)s subnets and "
                     "%(ports)s ports"),
        ...
        for count, network in enumerate(source_networks, 1):
            ...
            try:
                ...
                LOG.info(_LI("Created network %(count)s/%(total)s: %(net)s"),
                         {'count': count, 'total': total_num,
                          'net': created_net})
            except Exception as e:
            ...
            for subnet_id in network['subnets']:
                ...
                try:
                    ...
                    LOG.error(_LE("Failed to create subnet: %(subnet)s: "
                                  "%(e)s"),
                              {'subnet': subnet, 'e': e})
                ...
                if self.have_id(port['id'], dest_ports) is False:
                    if port['device_owner'] == 'network:router_gateway':
                        ...
                        try:
                            ...
                            LOG.info(_LI("Uplinked router %(rtr)s to external "
                                         "network %(net)s"),
                                     {'rtr': router_id,
                                      'net': port['network_id']})

                        except Exception as e:
                            LOG.error(_LE("Failed to add router gateway "
                                          "(%(port)s): %(e)s"),
                                      {'port': port, 'e': e})
                        continue
                    ...
                    if (port['device_owner'] == 'network:router_interface' and
                        ...
                        try:
                            ...
                            created_port = self.dest_neutron.create_port(
                                {'port': body})['port']
                            LOG.info(_LI("Created interface port %(port)s "
                                         "(subnet %(subnet)s, ip %(ip)s, mac "
                                         "%(mac)s)"),
                            ...
                            LOG.error(_LE("Failed to add router interface port"
                                          "(%(port)s): %(e)s"),
                                      {'port': port, 'e': e})
                        continue
                    ...
                    try:
                        ...
                        LOG.info(_LI("Created port %(port)s (subnet "
            ...
            for subnet in dhcp_subnets:
                try:
                    ...
                    LOG.error(_LE("Failed to enable DHCP on subnet "
                                  "%(subnet)s: %(e)s"),
    ...
    def migrate_floatingips(self):
        ...
        for count, source_fip in enumerate(source_fips, 1):
            ...
            try:
                ...
                LOG.info(_LI("Created floatingip %(count)s/%(total)s : "
                             "%(fip)s"),
